...
jsx_export void sow_web_jsx::js_write_header(std::stringstream& stream) {
	stream << "context.response.write = function write( val ) { if ( !val || val === null || val === undefined ) return;  /*if( typeof( val )===\"string\" ) val = val.trim();*/ context.response._write( val + '\\r\\n' );};\r\n";
...
int script_tag_parser::parse(template_result & tr,
	...
	//std::regex* ws = new std::regex("/^\\s*|\\s*$");
	//std::regex*tab = new std::regex("(?:\\t)");
	parser_info* info = new parser_info();
	js_parser* jsp = new js_parser();//(js_parser*)malloc(sizeof js_parser);
	info->is_tag_start = false;
	info->is_tag_end = true;
	for (std::sregex_token_iterator end; iter != end; ++iter) {
		info->tag_found = false;
		info->line = iter->str();
		//info->line = std::regex_replace(info->line, *tab, "");
		if (info->line.empty()) {
			//stream << jsp->result_key << " += '';";
			continue;
		}
		//i++;
		if (info->is_tag_end) {
	...
	//html_body = std::regex_replace(html_body, *ws, " ");
	//delete ws;
	//html_body = std::regex_replace(html_body, std::regex("(?:" + jsp->result_key + " \\+\\= '';)"), "");
	//delete jsp;
