...
#if !defined(_uws_app_wrapper)
#define _uws_app_wrapper
...
template <typename APP>
void uWS_App_run(const FunctionCallbackInfo<Value>& args) {
	APP* app = (APP*)args.Holder()->GetAlignedPointerFromInternalField(0);
	app->run();
}
...
void uWS_App(const FunctionCallbackInfo<Value>& args) {
	...
	appTemplate->PrototypeTemplate()->Set(String::NewFromUtf8(isolate, "run", NewStringType::kNormal).ToLocalChecked(), FunctionTemplate::New(isolate, uWS_App_run<APP>, args.Data()));
