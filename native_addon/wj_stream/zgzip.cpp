...
*/
#	include "zgzip.h"
//This function contain UTF-8-BOM
//If you want to disable UTF-8-BOM
//Use _strdup instead of copy_str
char* gzip::copy_str(const char* str, size_t len){
	char* buff = new char[len + sizeof(char)];
	strcpy_s(buff, len, str);
	buff[len] = '\000';
	return buff;
}
...
int gzip::deflate_file(
	...
	const char* output_path, int level,
	std::string& error
){
	...
	std::ifstream* source = new std::ifstream(input_path, std::ifstream::binary);
	if (!source->is_open()) {
		error = "Unable to open input file #";
		error.append(input_path);
		return FALSE;
	}
	std::ofstream* dest = new std::ofstream(output_path, std::ofstream::out | std::ofstream::binary);
	if (!dest->is_open()) {
	...
	if (ret != Z_STREAM_END) {
		...
		error = "stream should be completed";
		ret = FALSE;
	}
	else {
		ret = TRUE;
...
int gzip::inflate_file(
	...
	do {
		...
		strm->avail_in = (uInt)read_len;
		totalSize -= read_len;
		eof = totalSize == 0 || totalSize == std::string::npos ? TRUE : FALSE;
		strm->next_in = (Bytef*)in;
...
int gzip::gzip_deflate::f_open_file(const char* path) {
	this->f_close_file();
	this->_fs = new std::ofstream(path, std::ofstream::out | std::ofstream::binary);
	if (!this->_fs->is_open()) {
...
size_t gzip::gzip_deflate::f_write_file(const char* file_path, int do_flush) {
	if (this->_fs == NULL) {
...
int gzip::gzip_deflate::panic(char* error_msg) {
	return this->panic(const_cast<const char*>(error_msg), TRUE);
}
//[/Deflate==>Compress]
//[Inflate==>Decompress]
...
int gzip::gzip_inflate::f_open_file(const char* path) {
	this->f_close_file();
	this->_fs = new std::ofstream(path, std::ofstream::out | std::ofstream::binary);
	if (!this->_fs->is_open()) {
		return panic("Unable to open file....", -1);
...
size_t gzip::gzip_inflate::f_write_file(const char* file_path) {
	if (this->_fs == NULL) {
		return panic("FileStream not open yet....", -1);
	}
...
int gzip::gzip_inflate::panic(char* erro_msg){
	...
	_internal_error = new char[len + 1];
	strcpy_s(_internal_error, len, const_cast<const char*>(erro_msg));
	_is_error = TRUE;
	return _is_error;
}
//[/Inflate==>Decompress]